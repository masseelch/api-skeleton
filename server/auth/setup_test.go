...
func setup(t *testing.T) (c *ent.Client, v *validator.Validate, l *logrus.Logger, h *logrusTest.Hook) {
	...
	if _, err := c.Session.CreateBulk(
		// Life has expired.
		c.Session.Create().SetID(token.Token(lifeExpiredToken)).SetIdleTimeExpiredAt(time.Now().Add(10)).SetLifeTimeExpiredAt(time.Now().Add(-10)),
		// Idle has expired.
		c.Session.Create().SetID(token.Token(idleExpiredToken)).SetIdleTimeExpiredAt(time.Now().Add(10)).SetLifeTimeExpiredAt(time.Now().Add(-10)),
		// Valid but blocked user.
		c.Session.Create().SetID(token.Token(validTokenBlockedUser)).SetIdleTimeExpiredAt(time.Now().Add(time.Hour)).SetLifeTimeExpiredAt(time.Now().Add(time.Hour)).SetUserID(1),
		// Valid.
		c.Session.Create().SetID(token.Token(validToken)).SetIdleTimeExpiredAt(time.Now().Add(time.Hour)).SetLifeTimeExpiredAt(time.Now().Add(time.Hour)).SetUserID(2),
	).Save(ctx); err != nil {
		panic(err)
	}
