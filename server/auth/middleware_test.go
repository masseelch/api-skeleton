...
func TestGetTokenFromRequest(t *testing.T) {
	...
	assert.Equal(t, token.Token(validToken), getTokenFromRequest(req))

	// query param
	req = httptest.NewRequest(http.MethodPost, "/", nil)
	q := req.URL.Query()
	q.Add("auth", validToken)
	req.URL.RawQuery = q.Encode()
	assert.Equal(t, token.Token(validToken), getTokenFromRequest(req))

	// header precedence
	req.Header.Set("Authorization", invalidToken)
	assert.Equal(t, token.Token(invalidToken), getTokenFromRequest(req))
}
	...
	tests := []struct {
		...
		},
		{
			...
			func(t *testing.T, tt test) {
				...
				assert.NotNil(t, s)
				assert.Equal(t, token.Token(validToken), s.ID)
